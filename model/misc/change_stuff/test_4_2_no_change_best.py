...
main_data_formatted = main_data_formatted.query("change == 0")
main_data_formatted = main_data_formatted.reset_index(drop=True)
trial_counts = main_data_formatted.groupby('token_id').size()
trial_counts = dict(trial_counts)
print('hih')
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('model_results/test4_split/rules_prior_no_change_best.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior)
          with open('model_results/test4_split/rules_prior_label_no_change_best.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior_label)
          with open('model_results/test4_split/rules_post_map_no_change_best.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_map)
          with open('model_results/test4_split/rules_post_all_no_change_best.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all)
          with open('model_results/test4_split/rules_post_all_seed_no_change_best.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all_seed)



          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_process_models_no_change_best.csv')   # writing main data to new csv file including all relevant data for analysis
