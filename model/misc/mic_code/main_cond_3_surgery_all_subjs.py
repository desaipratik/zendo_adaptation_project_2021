...
####################### Custom imports ########################################
from mcmc_cond_3_new import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=10000):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          # label mismatch all subjects
          label_mismatch_probs = []

          # label mismatch single subject
          raw_probs_all_trials_lm = []
          fitted_taus_lm = []
          ll_model_lm= []           # negative log likelihood of observing participant data for a given rule (i.e. sum of all log ll for each scene for one rule)
          BICs_model_lm= []
          select_probs_lm = []
          raw_probs_all_trials_one_list_lm = []
          select_probs_all_trials_lm = []
          prior_resp_all_trials_lm = []
          fitted_taus_all_trials_lm = []
          ll_model_all_trials_lm= []
          BICs_model_all_trials_lm= []
          ...
          for data in main_data_formatted['data'][:n_rows]:
               ...
               rev_trials =  eval(main_data_formatted['prior_partner'][i])
               ...
               label_mismatch = [s and not p or p and not s for s,p in zip(prior_response,rev_trials)]
               label_mismatch = [int(s and not p) or int(not s and p) for s,p in zip(prior_response,label_mismatch)]
               ...
               full_training_dat_post_map = generalizations + label_dat_post_map
               full_training_dat_post_all = full_data + training_data
               all_labels = rev_trials + prior_response


               prior_probs = get_production_probs_prototype(full_training_dat_prior,'prior',cond='3',feat_only=False)
               Dwin_prior = prior_probs[0]
               feat_probs_prior = prior_probs[1]

               # #same for both post map and post all
               post_probs = get_production_probs_prototype(full_training_dat_post_all ,'post',cond='3', truth=rev_trials,feat_only=False, )
               ...
               # print('prior_labesl')
               for sample in range(n_1):
                    prior_label_df_and_details = mcmc_sampler(productions,
                                                                     replacements,
                                                               label_dat_post_map,
                                                               ground_truth_labels=rev_trials,
               ...
               max_a_posteriori_prior_label = df_prior_labels['rulestring'].mode()[0]     # getting the mode of the prior chain first to initialise the post map chain
               ...
               prior_rule_strings_label = df_prior_labels['rulestring'].tolist()
...
#                     # print(p_follow_rule_prior)
               ...
               BICs_model_prior.append(-2 * fitted_results_mod_prior[1] + 1 * np.log(8))
                ...
                # for label mismatch probs
               label_mismatch_probs.append(label_mismatch)
               raw_probs_all_trials_lm.append(label_mismatch)
               fitted_tau_lm = minimize(get_tau, 1, bounds=[(0.01, 10000.00)],args=(label_mismatch,posterior_response), method='L-BFGS-B')
               fitted_taus_lm.append(fitted_tau_lm.x[0])
               fitted_results_mod_lm = fitted_probs(fitted_tau_lm.x[0], label_mismatch,posterior_response)
               select_probs_lm.append(fitted_results_mod_prior[0])
               ll_model_lm.append(fitted_results_mod_prior[1])
               BICs_model_lm.append(-2 * fitted_results_mod_prior[1] + 1 * np.log(8))
               ...
               if n_trials_counter % n_trials == 0:
                    ...
                    # label mismatch ]
                    raw_probs_all_trials_lm_1 = [prob for sublist in raw_probs_all_trials_lm[i-n_trials+1:] for prob in sublist]
                    post_resp_all_trials_lm_1 = [response for sublist in post_resp_all_trials_post_map[i-n_trials+1:] for response in sublist]

                    overall_fitted_tau_lm = minimize(get_tau, 1, bounds=[(0.01, 10000.00)],args=(raw_probs_all_trials_lm_1, post_resp_all_trials_lm_1), method='L-BFGS-B')
                    fitted_results_all_trials_mod_lm = fitted_probs(overall_fitted_tau_lm.x[0], raw_probs_all_trials_lm_1 ,  post_resp_all_trials_lm_1)
                    ...
                    post_all_lm = compare_BIC(fitted_results_all_trials_mod_lm[1], fitted_results_all_trials_baseline_post_all[1], 8*n_trials)
...
#                          # print(fitted_taus_all_subjects_prior)
                        ...
                        # label mismatch
                         label_mismatch_probs_all_subjects = [prob for sublist in label_mismatch_probs for prob in sublist]
                         label_mismatch_resp_all_subjects = [response for sublist in post_resp_all_trials_post_all for response in sublist]

                         fitted_tau_all_subjects_label_mismatch = minimize(get_tau, 1, bounds=[(0.01, 10000.00)],args=(label_mismatch_probs_all_subjects,label_mismatch_resp_all_subjects), method='L-BFGS-B')
                         fitted_results_all_subjects_label_mismatch = fitted_probs(fitted_tau_all_subjects_label_mismatch.x[0],  label_mismatch_probs_all_subjects,  label_mismatch_resp_all_subjects )

                         fitted_taus_all_subjects_label_mismatch = []
                         fitted_taus_all_subjects_label_mismatch.append(fitted_tau_all_subjects_label_mismatch.x[0])

                         select_probs_all_subjects_label_mismatch = []
                         select_probs_all_subjects_label_mismatch.append(fitted_results_all_subjects_label_mismatch[0])

                         ll_model_all_subjects_label_mismatch  = []
                         ll_model_all_subjects_label_mismatch.append(fitted_results_all_subjects_label_mismatch[1])

                         BICs_model_all_subjects_label_mismatch = []
                         BICs_model_all_subjects_label_mismatch.append(-2 * fitted_results_all_subjects_label_mismatch[1] + 1 * np.log(8 * len(main_data_formatted['rule_name'][:n_rows])))
...
#                          # print(BICs_baseline_all_subjects_prior)
                         ...
                         lm_mod_select_all_subj =  hard_max_selections(raw_probs_all_trials_lm)
                         lm_mod_select_all_subj_1 = [prob for sublist in lm_mod_select_all_subj for prob in sublist]
                         ...
                         lm_acc_all_subj = sum([a and b or not a and not b for a, b in zip(gt_all_subjects, lm_mod_select_all_subj_1)]) / len(lm_mod_select_all_subj)
...
#                          # print(len(fitted_taus_prior))
                    ...
                    for trial in range(n_trials):
                         ...
                         fitted_taus_all_trials_lm.append(overall_fitted_tau_lm.x[0])
                         select_probs_all_trials_lm.append(fitted_results_all_trials_mod_lm[0])
                         ll_model_all_trials_lm.append(fitted_results_all_trials_mod_lm[1])
                         BICs_model_all_trials_lm.append(-2 * fitted_results_all_trials_mod_lm[1] + 1 * np.log(8*n_trials))
                         # ll_baseline_all_trials_.append(fitted_results_all_trials_baseline_post_map[1])
                    ...
                    lm_mod_select =  hard_max_selections(raw_probs_all_trials_lm_1)
                    ...
                    lm_acc = sum([a and b or not a and not b for a, b in zip(gt_all, lm_mod_select)]) / len(lm_mod_select)
                    ...
                    for acc in range(n_trials):
                         ...
                         lm_accs_single.append(sum([a and b or not a and not b for a, b in zip(gt, lm_mod_select[low_bound:up_bound])]) / 8)
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          main_data_formatted['fitted_taus_all_trials_lm'] = fitted_taus_all_trials_lm
          # main_data_formatted['select_probs_all_subjects_post_all'] = select_probs_all_subjects_post_all
          main_data_formatted['ll_model_all_trials_lm'] = ll_model_all_trials_lm
          main_data_formatted['BICs_model_all_trials_lm'] = BICs_model_all_trials_lm
          main_data_formatted['fitted_taus_all_subjects_lm'] = fitted_taus_all_subjects_label_mismatch * len(main_data_formatted['rule_name'][:n_rows])
          # main_data_formatted['raw_probs_all_subjects_lm'] = [raw_probs_all_subjects]
          main_data_formatted['ll_model_all_subjects_lm'] = ll_model_all_subjects_label_mismatch * len(main_data_formatted['rule_name'][:n_rows])
          main_data_formatted['BICs_model_all_subjects_lm'] = BICs_model_all_subjects_label_mismatch * len(main_data_formatted['rule_name'][:n_rows])
          ...
          main_data_formatted['lm_acc'] = lm_accs
          main_data_formatted['lm_acc_single'] = lm_accs_single
          ...
          main_data_formatted['all_sub_lm_acc'] = lm_acc_all_subj
          ...
          with open('model_results/norm_3/rules_prior_104_all.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior)
          # with open('model_results/norm_3/rules_prior_label.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_prior_label)
          # with open('model_results/norm_3/rules_post_map.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_post_map)
          # with open('model_results/norm_3/rules_post_all.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_post_all)
          with open('model_results/norm_3/rules_post_all_seed_104_all.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all_seed)



          main_data_formatted.to_csv('model_results/normative_res_cond_three_' + str(rep) + '10k_104.csv')   # writing main data to new csv file including all relevant data for analysis
