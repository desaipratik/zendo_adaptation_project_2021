...
main_data_formatted = pd.read_csv('main_data_formatted_cond_one_surgery.csv')  # getting the preprocessed data file
...
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=10000):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data_prior'][:n_rows]:
               ...
               subj_rule = main_data_formatted['prior_resp'][1]
               bv_prior = eval(main_data_formatted['bound_vars'][1])
               # print(subj_rule)
               # print(bv_prior)
               ...
               prior_rule_strings_label = [subj_rule]
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('model_results/norm_1/rules_prior_450.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior)
          # with open('model_results/norm_1/rules_prior_label_450.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_prior_label)
          # with open('model_results/norm_1/rules_post_map_450.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_post_map)
          with open('model_results/norm_1/rules_post_all_450.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all)
          # with open('model_results/norm_1/rules_post_all_seed_450.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_post_all_seed)


          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_10k__450.csv')   # writing main data to new csv file including all relevant data for analysis
