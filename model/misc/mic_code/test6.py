...
####################### Custom imports ########################################
from mcmc_cond_3_new_test import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
#                     # print('dropped abvoe ')
                    ...
                    row_count+=1
               # else:
               #     df_post_map = pd.DataFrame({'rulestring': [subj_rule] * 1000})
...
#                # print(df_post_map['rulestring'])
               ...
               print(len(post_map_rule_strings))
...
#                # print(df_post_all)
               ...
               for sample in range(n_1):
                    df_post_all_seed = df_post_all_seed.append(mcmc_sampler_map(productions,
                                                           ...
                                                           Dwin=Dwin_post,test_data=test_training_data,
                                                                         feat_probs=feat_probs_post,
                                                           iter = 1000,
...
#                          # print(fitted_taus_all_subjects_prior)
                         ...
                         fitted_tau_all_subjects_post_all = minimize(get_tau, 1, bounds=[(0.1, 100.00)],args=(raw_probs_all_subjects_post_all,resp_all_subjects_post_all), method='L-BFGS-B')
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('model_results/test6/rules_prior_chain.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior)
          with open('model_results/test6/rules_prior_chain_label.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior_label)
          with open('model_results/test6/rules_post_map_chain.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_map)
          with open('model_results/test6/rules_post_all_chain.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all)
          with open('model_results/test6/rules_post_all_seed_chain.txt', 'w') as filehandle:
          ...
          main_data_formatted.to_csv('model_results/normative_res_cond_three_' + str(rep) + 'process_models_chain.csv')   # writing main data to new csv file including all relevant data for analysis
