...
####################### Custom imports ########################################
from mcmc_cond_1_new_test_change_seq_ib_15 import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
# #           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('tr_counts_only_change_15'+str(rep)+'.txt', 'w') as f_tr:
              for item_tr in n_iter_tr:
                  f_tr.write("%s\n" % item_tr)

          with open('ts_counts_only_change_15'+str(rep)+'.txt', 'w') as f_ts:
              for item_ts in n_iter_ts:
                  f_ts.write("%s\n" % item_ts)



          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_process_models_inverse_binomial_only_change_15.csv')   # writing main data to new csv file including all relevant data for analysis
