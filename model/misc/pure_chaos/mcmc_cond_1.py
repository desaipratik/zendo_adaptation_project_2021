...
def resample_rule_2(parse_tree, productions):
    ...
    # print('parsetree')
    # print(parse_tree)
    keys = [key for key in list(parse_tree.keys()) if key in ["S"]] # keeping only non-terminals
...
#         # print(fixed_d)
        ...
        contents = file.read()
        production_dict = ast.literal_eval(contents)
        production_dict = production_dict['exDict']
#         # print(production_dict)
        file.close()

        sub_tree = resample_rule_2(production_dict, productions)
        # len_sub_tree = sum([len(sub_tree[key]) for key in list(sub_tree.keys())])  # len / size of the subtree
#         # print(prod_d)
#         # print('prod_d')
#         # print(sub_tree)
#         # print('sub_tree')
        ...
        rules.append(sub_t[0])
        prod_dicts.append(sub_tree)
...
#             # print(t_prime["rule"])
            res_t.append(eval(t["rule"]))             # evaluating the rules against X
            # print(sub_t)
            res_t_prime.append(eval(sub_t[0]))
            ground_truth.append(data[i_2]['follow_rule'])
        ...
        for i_4 in range(8, len(data)):
            ...
            res_t_process.append(eval(t["rule"]))             # evaluating the rules against X
            res_t_prime_process.append(eval(sub_t[0]))
            # ground_truth_trials_rev.append(data[i_4]['follow_rule'])
        z_t_process = [a and b or not a and not b for a, b in zip(ground_truth_labels, res_t_process)]  # comparing results with ground truth
        z_t_prime_process = [a and b or not a and not b for a, b in zip(ground_truth_labels, res_t_prime_process)]
        ...
        # nt = sm
        # # 5) putting the above together to compute acceptance probability(np.log(p_t_prime) - np.log(p_t)) +v
        nt_p = sum([len(sub_tree[k]) for k in sub_tree if k in ["S", "A", "B", "C", ]])
        nt = sum([len(production_dict[k]) for k in production_dict if k in ["S", "A", "B", "C", ]])

        p_acceptance = (np.log(ll_t_prime) - np.log(ll_t))
                       # + np.log(nt) - np.log(nt_p)
                       # + (np.log(deriv_t_prime) - np.log(deriv_t))
...
#         # print('now_t_prime')
        ...
        if i >= 4:
            ...
            maxscore = surgery_t_res[max_acceptance]
            # print(surgery_t_res)
            # print('results')
            # print(t['rule'])

#             # print(surgery_t[max_acceptance])
            if maxscore > 1:
                t['rule'] = rules[max_acceptance]
            #     print(t['rule'])
            #     print('revised')
            # # print(t['rule'])
            df_rules["rulestring"][row] = t["rule"]
