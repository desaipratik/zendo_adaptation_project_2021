...
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, n_1=1, n_2=10):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of iterations)
     ...
     for data in main_data_formatted['data'][:100]:
         ...
         # sum(check_change_impact_single_feat(follow,check_feat_change(grounded)))+sum(list(kkk['3'].values()))]
          ...
          s = [sum(check_change_impact_single_feat(follow,check_feat_change(colours))),
         sum(check_change_impact_single_feat(follow,check_feat_change(sizes))),
         sum(check_change_impact_single_feat(follow,check_feat_change(orientations))),
         sum(check_change_impact_single_feat(follow,check_feat_change(grounded)))]
...
#         print(k)
          ...
          Dwin = [0.25+k[0],0.25+k[1],0,0,0,0.25+k[2],0.25+k[3]]
          print(Dwin)
          ...
          # prior chain
          df_prior = mh_mcmc_sampler_res(productions, trials,Dwin=Dwin,ground_truth=False, iter = 0)[0] # creating empty data for first initial rules
     ...
     main_data_formatted.to_csv('summary_adaptive_values_pruned_grammar_1_remains.csv')   # writing main data to new csv file including all relevant data for analysis
