...
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, rules_dict, n_rep = 5, n_1=1, n_2=100):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data']:
               ...
               df_prior = mh_mcmc_sampler_res(productions, full_training_dat,Dwin=Dwin,feat_probs=feat_probs,ground_truth=False, iter = 0)[0] # creating empty data for first initial rules
          ...
          main_data_formatted.to_csv('boolean_adaptive_pcfg_all_rules' + str(rep) + '.csv')   # writing main data to new csv file including all relevant data for analysis
