...
####################### Custom imports ########################################
from mcmc_adaptive_cond_2 import mh_mcmc_sampler_res
...
main_data_formatted = pd.read_csv('main_data_formatted_cond_2.csv')  # getting the preprocessed data file
...
def predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep = 5, n_1=1, n_2=2):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data']:
               ...
               Dwin_prior = get_production_probs(full_training_dat_prior,'prior', cond='3')[0]
               feat_probs_prior = get_production_probs(full_training_dat_prior,'prior', cond='3')[1]
          ...
          main_data_formatted.to_csv('model_results/adaptive_res_1_100_cond_2_' + str(rep) + '.csv')   # writing main data to new csv file including all relevant data for analysis
