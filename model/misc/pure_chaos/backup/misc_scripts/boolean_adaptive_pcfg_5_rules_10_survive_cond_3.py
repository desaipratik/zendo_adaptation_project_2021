...
def predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep = 5, n_1=1, n_2=100):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data_prior']:
               ...
               df_prior.to_csv("boolean_adatpive_10_survive/8_training_data/"+token_id + "_" + rule_name + "_prior_cond_3.csv")       # storing a separate df including rules for each participant
               df_post_map.to_csv("boolean_adatpive_10_survive/8_training_data/"+token_id + "_" + rule_name + "_post_map_cond_3.csv")
               df_post_all.to_csv("boolean_adatpive_10_survive/8_training_data/"+token_id + "_" + rule_name + "_post_all_cond_3.csv")
          ...
          main_data_formatted.to_csv('boolean_adatpive_10_survive/8_training_data/boolean_adaptive_pcfg_5_rules_5_penalisation_cond_3' + str(rep) + '.csv')   # writing main data to new csv file including all relevant data for analysis
