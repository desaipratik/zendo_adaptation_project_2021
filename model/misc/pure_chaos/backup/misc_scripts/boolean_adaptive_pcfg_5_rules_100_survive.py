...
print(pd.__version__)  # needs to be a relatively recent version (older versions coming with python 2.7 don't work)
...
print(len(main_data_formatted['rule_name']))  # remaining number of trials (104/200)
...
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep = 5, n_1=1, n_2=1000):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data']:
               ...
               Dwin_post = get_production_probs(full_training_dat_post_all ,'post',truth=truth_partner)[0]
               feat_probs_post = get_production_probs(full_training_dat_post_all ,'post',truth=truth_partner)[1]
               ...
               # prior
               df_prior = mh_mcmc_sampler_res(productions, full_training_dat_prior,Dwin=Dwin_prior,feat_probs=feat_probs_prior,ground_truth=False, iter = 0)[0] # creating empty data for first initial rules
               prior_details = [] # details for prior rules are needed to run the map mcmc chain which starts with the map rule of the prior

               # post map chain
               df_post_map = mh_mcmc_sampler_res(productions, full_training_dat_post_map , Dwin=Dwin_post,feat_probs=feat_probs_post ,ground_truth=False, iter = 0)[0] # creating empty data for revised rules

               # post all chain
               df_post_all = mh_mcmc_sampler_res(productions, full_training_dat_post_all, Dwin=Dwin_post,feat_probs=feat_probs_post ,iter = 0)[0] # creating empty data for all rules
               ...
               for sample in range(n_1):
                    prior_df_and_details = mh_mcmc_sampler_res(productions,
                                                               ...
                                                               Dwin=Dwin_prior,feat_probs=feat_probs_prior,
                                                               ground_truth=False,
                                                               iter = n_2,
                                                               type='prior',
                                                               out_penalizer=5)
               ...
               for sample in range(n_1):
                    df_post_map = df_post_map.append(mh_mcmc_sampler_res(productions,
                                                             ...
                                                             Dwin=Dwin_post,feat_probs=feat_probs_post,
               ...
               for sample in range(n_1):
                    df_post_all = df_post_all.append(mh_mcmc_sampler_res(productions,
                                                           full_training_dat_post_all,feat_probs=feat_probs_post,
                                                                      Dwin=Dwin_post,
                                                           ground_truth=eval(main_data_formatted['prior_partner'][i]),
                                                           iter = n_2,
                                                           type='posterior_all',
          ...
          main_data_formatted.to_csv('boolean_adatpive_100_survive/8_training_data/boolean_adaptive_pcfg_5_rules_5_penalisation' + str(rep+10) + '.csv')   # writing main data to new csv file including all relevant data for analysis

          rep+=1

predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep=15)
