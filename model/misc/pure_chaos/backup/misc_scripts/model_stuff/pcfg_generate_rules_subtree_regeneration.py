...
def mh_mcmc_sampler(productions,
                    data,
                    ground_truth = None,
                    start="S",
    ...
    for i in range(iter):
        ...
        # t_prime = ob.generate(productions, start, [], start_frame)
        ...
        # doing this for the 8 generalizations from the partner
            ...
            for i_2 in range(0, len(data)):
                ...
                for i_3 in range(0, len(data[i_2]['ids'])):  # looping over the number of objects in each scene
                    ...
                    object = {"id": data[i_2]['ids'][i_3], "colour":  data[i_2]['colours'][i_3] , "size":  data[i_2]['sizes'][i_3], "xpos":  data[i_2]['xpos'][i_3],
                    "ypos":  data[i_2]['ypos'][i_3], "rotation":  data[i_2]['rotations'][i_3], "orientation": compute_orientation(data[i_2]['rotations'][i_3]),
        ...
        # doing this for all 16 data points together
            ...
            for i_4 in range(0, len(data[8:])):
                ...
                for i_5 in range(0, len(data[i_4]['ids'])):  # looping over the number of objects in each scene
                    ...
                    # getting the properties for each object (triangle / cone) in the scene
                    object = {"id": data[i_4]['ids'][i_5], "colour":  data[i_4]['colours'][i_5] , "size":  data[i_4]['sizes'][i_5], "xpos":  data[i_4]['xpos'][i_5],
                    "ypos":  data[i_4]['ypos'][i_5], "rotation":  data[i_4]['rotations'][i_5], "orientation": compute_orientation(data[i_4]['rotations'][i_5]),
        ...
        df_rules["prod_d"][i] = t["prod_d"]
        df_rules["results"][i] = z_t
        df_rules["gt_trials"][i] = ground_truth_trials
