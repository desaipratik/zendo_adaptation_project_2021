...
####################### Custom imports ########################################
from pcfg_generate_rules_subtree_regeneration_restricted_grammar import mh_mcmc_sampler
...
for data in main_data_formatted['data'][26:27]:
    trials = eval(data)[:8]
    ...
    for i in trials:
        ...
        sizes.append(i['sizes'])
        grounded.append(i['grounded'])
        follow.append(i['follow_rule'])
        contact.append(i['contact'])
        xpos.append(i['xpos'])
        ypos.append(i['ypos'])
        rotations.append(i['rotations'])
        orientations.append(i['orientations'])
    ...
    s = [sum(check_change_impact_single_feat(follow,check_feat_change(colours)))+sum(list(kkk['0'].values())),
          ...
          sum(check_change_impact_single_feat(follow,check_feat_change(transform_xpos(xpos))))+sum(list(kkk['4'].values())),
          sum(check_change_impact_single_feat(follow,check_feat_change(transform_ypos(ypos))))+sum(list(kkk['5'].values())),
          sum(check_change_impact_single_feat(follow,check_feat_change(orientations)))+sum(list(kkk['6'].values()))]
    ...
    print(s)
    print(k)
    k = [float(i)/sum(k) for i in k]
    print(k)

    df_prior = mh_mcmc_sampler(productions,trials, ground_truth=False, iter = 0, Dwin=k)[0] # creating empty data for first initial rules
    for i in range(1):
        prior_df_and_details = mh_mcmc_sampler(productions, trials, ground_truth=False, iter =100 ,type='prior',out_penalizer=1, Dwin=k)
...
#     contact.append(i['contact'])
#     xpos.append(i['xpos'])#
#     ypos.append(i['ypos'])#      # computing additional variables for a single trial for subjects prior responses
#     rotations.append(i['rotations'])#      fitted_taus_prior = []        # fitted temperature parameters for each trial for each participant (dictates soft vs hard maximisation)
#     orientations.append(i['orientations'])#      raw_probs_prior = []          # raw probabilities for each scene (i.e. if they follow a rule or not)
# #      print('####################')   #      select_probs_prior = []       # selection probabilities for each scene based on tau
...
#      for data in main_data_formatted['data'][:5]:
#
#
...
#                     object = {"id": gen['ids'][i_3], "colour":  gen['colours'][i_3] , "size":  gen['sizes'][i_3], "xpos":  gen['xpos'][i_3],
#                     "ypos":  gen['ypos'][i_3], "rotation":  gen['rotations'][i_3], "orientation": compute_orientation(gen['rotations'][i_3]),
...
#           df_prior.to_csv("sampled_rules_1_05/"+token_id + "_" + rule_name + "_prior.csv")       # storing a separate df including rules for each participant
#           df_post_map.to_csv("sampled_rules_1_05/"+token_id + "_" + rule_name + "_post_map.csv")
#           df_post_all.to_csv("sampled_rules_1_05/"+token_id + "_" + rule_name + "_post_all.csv")
#
...
#      main_data_formatted = main_data_formatted[:5]
...
#
#
#      main_data_formatted.to_csv('summary_1_05_50k_1.csv')   # writing main data to new csv file including all relevant data for analysis
#
#
#
