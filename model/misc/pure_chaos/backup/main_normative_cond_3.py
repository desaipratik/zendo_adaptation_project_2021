...
####################### Custom imports ########################################
from mcmc_normative_cond_three import mh_mcmc_sampler_res
...
main_data_formatted = pd.read_csv('main_data_formatted_cond_three.csv')  # getting the preprocessed data file
...
print('cat')
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep = 5, n_1=5, n_2=5):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data'][:n_rows]:
               ...
               post_probs = get_production_probs_prototype(full_training_dat_post_all ,'post',cond='3', feat_only=False, truth=truth_partner)
               ...
               for sample in range(5):
                    # print(df_prior['rulestring'].mode()[0])
                    map_index_label = np.random.randint(len(df_prior_labels['rulestring']))
          ...
          main_data_formatted.to_csv('model_results/normative_res_con_three_' + str(rep) + '_new_models.csv')   # writing main data to new csv file including all relevant data for analysis
