...
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, n_1=1, n_2=50000):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of iterations)
     ...
     for data in main_data_formatted['data'][50:100]:


          # getting name and id to create a unique csv file for each mcmc chain for each subject and trial
          rule_name = main_data_formatted['rule_name'][i+50]
          token_id = main_data_formatted['token_id'][i+50]

          # getting subjects prior and posterior responses
          prior_response = eval(main_data_formatted['prior'][i+50])
          posterior_response = eval(main_data_formatted['posterior'][i+50])
          ...
          acc_partner = sum([a and b or not a and not b for a,b in zip(gt, eval(main_data_formatted['prior_partner'][i+50]))]) / 8

          for sample in range(n_1):
               df_post_map = df_post_map.append(mh_mcmc_sampler(productions,
                                                        generalizations,
                                                        ground_truth=eval(main_data_formatted['prior_partner'][i+50]),
          ...
          df_prior.to_csv("sampled_rules_5_05/"+token_id + "_" + rule_name + "_prior.csv")       # storing a separate df including rules for each participant
          df_post_map.to_csv("sampled_rules_5_05/"+token_id + "_" + rule_name + "_post_map.csv")
          df_post_all.to_csv("sampled_rules_5_05/"+token_id + "_" + rule_name + "_post_all.csv")
     ...
     main_data_formatted = main_data_formatted[50:100]
     ...
     main_data_formatted.to_csv('summary_5_05_50k_2.csv')   # writing main data to new csv file including all relevant data for analysis
