...
####################### Custom imports ########################################
from mcmc_cond_1_new_test import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
main_data_formatted = pd.read_csv('normative_res_con_one_second_rule.csv')  # getting the preprocessed data file
...
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=5):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          # global X
          #
          # X = [{'id': 0, 'colour': 'red', 'size': 2, 'xpos': 4, 'ypos': 2, 'rotation': 1.5, 'orientation': 'lhs', 'contact': [[0,2],[2,0]], 'grounded': 'yes'},
                # {'id': 1, 'colour': 'blue', 'size': 1, 'xpos': 7, 'ypos': 4, 'rotation': 3.5, 'orientation': 'rhs', 'contact': [[1]], 'grounded': 'yes'},
                # {'id': 2, 'colour': 'green', 'size': 3, 'xpos': 1, 'ypos': 5, 'rotation': 2.5, 'orientation': 'rhs', 'contact': [[0,2],[2,0]], 'grounded': 'yes'}]
          ...
          for data in main_data_formatted['data_prior'][:n_rows]:
               ...
               bv_post = eval(main_data_formatted['bound_vars_post'][i])
               ...
               correct_rule_perc_prior.append(prior_rule_strings.count(subj_rule_post))
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_checks_sec_rule.csv')   # writing main data to new csv file including all relevant data for analysis
