...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('model_results/test4/rules_prior_chain_clean_allbest.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior)
          with open('model_results/test4/rules_prior_label_chain_clean_allbest.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_prior_label)
          with open('model_results/test4/rules_post_map_chain_clean_allbest.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_map)
          with open('model_results/test4/rules_post_all_chain_clean_allbest.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all)
          with open('model_results/test4/rules_post_all_seed_chain_clean_allbest.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all_seed)



          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_process_models_only_allbest.csv')   # writing main data to new csv file including all relevant data for analysis
