...
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted,n_rep=1, n_1=1, n_2=1000):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of iterations)
     ...
     #  looping over each trial for each subject (n_trials * n_subjects iterations and run mcmc chains)
          for data in main_data_formatted['data']:
               ...
               # post map chain
               df_post_map = mh_mcmc_sampler(productions, trials, ground_truth=False, iter = 0)[0] # creating empty data for revised rules

               # post all chain
               df_post_all = mh_mcmc_sampler(productions, trials, ground_truth=False, iter = 0)[0] # creating empty data for all rules
     ...
     ################ APPENDING ALL DATA TO MAIN DATA FRAME ########################
          ...
          main_data_formatted.to_csv('full_comparison_100_survive/8_training_data/full_comparison_5_penalisation' + str(rep) + '.csv')   # writing main data to new csv file including all relevant data for analysis



predicted_selections(main_data_formatted,n_rep=10)
