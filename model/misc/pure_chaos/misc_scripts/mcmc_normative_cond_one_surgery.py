...
C = ['ob.equal(xN, D)']
D = {"colour": ["'red'", "'blue'", "'green'"], "size": [1, 2, 3],
...
productions = {"S": S, "A": A, "B": B, "C": C, "D": D, "E": E, "J": J, "K": K, "L": L, "M": M}
...
def reproduce_rule(parse_tree, productions,start="S"):
        ...
        for i in range(0, len(srule)):
            ...
            if srule[i] == 'K':
                ...
                if "K" not in recode.keys():
                    return rule, bound_vars, recode, recode
                else:
...
parse_tree = {'S': [2], 'A': [0], 'L': [2], 'B': [2, 2, 2, 1, 2, 0, 1, 0, 1, 1, 2, 0, 0, 0], 'M': [0], 'J': [0, 0, 0, 1], 'C': [0, 0, 0, 0, 0], 'D': [1, 5, 1, 5, 5], 'D1': [1, 1], 'D5': [3, 2, 2]}
    # {'S': [2], 'A': [0], 'L': [2], 'B': [0], 'C': [0], 'M': [0], 'D': [0], 'D0': [1]}

for kl in range(1):
    sub_tree = {'S': [2], 'A': [0], 'L': [2], 'B': [2, 2, 2, 1, 2, 0, 1, 0, 1, 1, 2, 0, 0, 0]}

    a = reproduce_rule(sub_tree,productions)
