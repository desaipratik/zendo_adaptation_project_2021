...
####################### Custom imports ########################################
from mcmc_adaptive_cond_3 import mh_mcmc_sampler_res
...
def predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep = 5, n_1=1, n_2=2):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data_prior']:
               ...
               # print(prior_details[0])
               # print(prior_details[map_index][str(map_index)])
               # print('tendaaai')
               # print(df_prior['rulestring'].mode()[0])
               # print(prior_details[map_index][str(map_index)])
               for sample in range(n_1):
                    ...
                    df_post_map = df_post_map.append(mh_mcmc_sampler_res(productions,
                                                             ...
                                                             start_frame=prior_details[map_index][str(map_index)],
                                                             iter = n_2,
                                                             type='posterior_map',
                                                             prod_l=df_prior['prod_l'][map_index],
                                                             prod_d=df_prior['prod_d'][map_index],
                                                                         row=sample,
                                                             out_penalizer=10000)[0])
               #
   ...
   iter_1 = n_1,
          ...
          main_data_formatted.to_csv('model_results/adaptive_res_1_100_cond_3_' + str(rep) + '.csv')   # writing main data to new csv file including all relevant data for analysis
