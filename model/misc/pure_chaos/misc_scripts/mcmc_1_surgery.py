...
def merge_two_dicts(x, y):
    """ returns a single dictionary including all common keys and values of x and y
    where x and y are both dictionaries with single lists as values"""
    z = dict(x)
    for k, v in y.items():
...
def resample_rule(parse_tree):
    ...
    keys = [key for key in list(parse_tree.keys()) if key in ["S", "A", "B"]] # keeping only non-terminals
    key = rd.choice(keys)
    # key="B"
...
def resample_rule_2(parse_tree, productions):
    ...
    d2 = None
    a = parse_tree["B"]
    keys = [key for key in list(parse_tree.keys()) if key in ["S", "B", "D", "J", "L"]] # keeping only non-terminals
    ...
    for i in range(iter):
        ...
        if type == 'posterior_map':
            ...
            t_prime = ob.generate_res(productions, sub_t[0], sub_t[1], t["prec"].loc[
                                                               0:len_sub_tree:],feat_probs=feat_probs,Dwin=Dwin)  # arguments: 1) sub_t[0] = sub rule, 2) sub_t[1] = sub bound vars,
            ...
            # t_prime["prod_l"] = ob.probs_list(reproduce_full_prod_dict(new_parse_tree, productions))   # s_list(reproduce_full_prod_dict(new_parse_tree, productions))   # same for production list
            t_prime["rule"] = sub_t[0]

            # print(t)

        else:
            sub_tree = resample_rule(t["prod_d"])
            len_sub_tree = sum([len(sub_tree[key]) for key in list(sub_tree.keys())])  # len / size of the subtree
        ...
        #                   **new_sub_tree}  # full parse tree for the new rule (t_prime). NOTE: the sub_tree part is identical to t
            ...
            # print('###PRECSTUFF###')
            # print(t["prec"].loc[0:len_sub_tree:])
            # print('###TPRIME###')
            # print(t_prime["rule"])
            # print(t_prime["prod_d"])
            # print(t_prime["prec"])
        ...
        if type == 'posterior_map':
            ...
            p_acceptance = (np.log(ll_t_prime) - np.log(ll_t)) + (np.log(deriv_t_prime) - np.log(deriv_t))
                           # + (np.log(deriv_t_prime) - np.log(deriv_t)) +  (np.log(nt_t) - np.log(nt_t_prime))
            # if np.log(ll_t_prime) + np.log(deriv_t_prime) > np.log(ll_t) + np.log(deriv_t):
            if np.log(np.random.rand()) < p_acceptance:
                if nt_t_prime < 2 * nt_t:
                    t = t_prime  # if accepted, the new rule becomes the new basis for comparison (t)
                    z_t = z_t_prime  # and the results are the results from the new rule
...
def mh_mcmc_sampler_res_map(productions,
                        data,
                        row=1,
                        ground_truth_labels = None,
                        start="S",
    ...
    for i in range(iter):
        ...
        t_prime = ob.generate_res(productions, sub_t[0], sub_t[1], t["prec"].loc[
                                                               0:len_sub_tree-1:],feat_probs=feat_probs,Dwin=Dwin)  # arguments: 1) sub_t[0] = sub rule, 2) sub_t[1] = sub bound vars,
        new_sub_tree = t_prime["prod_d"]
        new_parse_tree = merge_two_dicts(sub_tree, new_sub_tree)
        t_prime["prod_d"] = new_parse_tree
        t_prime["prod_l"] = ob.probs_list(reproduce_full_prod_dict(new_parse_tree, productions))
        surgery_t.append(t_prime)
        rules.append(t_prime['rule'])
        prod_dicts.append(t_prime['prod_d'])
        ...
        surgery_t_res.append(p_acceptance)
        ...
        if i >= .8 * iter:
            print(surgery_t_res)
            max_acceptance = surgery_t_res.index(max(surgery_t_res))
            print(max_acceptance)
            print(max_acceptance)
            print(prod_dicts)
            print(rules)
            print(t["rule"])
            print(t['prod_d'])
            print('init rule above')
            if p_acceptance > np.log(1):
                t = surgery_t[max_acceptance]
                print('revised')
            print(t["rule"])
            print('potentially revised rule above')
            df_rules["rulestring"][row] = t["rule"]
            df_rules["li"][int(row)] = np.prod(t["prec"]["li"])
            df_rules["productions"][int(row)] = t["prod_d"]
            df_rules["n_pds"][int(row)] = len(t["prec"])
