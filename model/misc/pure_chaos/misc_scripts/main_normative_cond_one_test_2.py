...
####################### Custom imports ########################################
from mcmc_normative_cond_one import mh_mcmc_sampler_res
...
main_data_formatted = pd.read_csv('main_data_formatted_cond_one.csv')  # getting the preprocessed data file
...
print('cat')
####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep = 5, n_1=5, n_2=20):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data_prior']:
               ...
               prior_probs = get_production_probs_prototype(full_training_dat_prior,'prior',feat_only=True)
               ...
               post_probs = get_production_probs_prototype(full_training_dat_post_all ,'post',cond='1', feat_only=True)
               ...
               df_prior_labels = mh_mcmc_sampler_res(productions, label_dat_post_map,Dwin=Dwin_prior,ground_truth=False, iter = 0)[0] # creating empty data for first initial rules
               ...
               print('prior_labesl')
               for sample in range(n_1):
                    prior_label_df_and_details = mh_mcmc_sampler_res(productions,
                                                               label_dat_post_map,
                                                               ground_truth=prior_response,
                                                               iter_1 = n_1,
                                                               Dwin=Dwin_prior,
                                                               feat_probs=feat_probs_prior,
                                                               iter = n_2,
               ...
               for sample in range(n_1):
                    ...
                    df_post_map = df_post_map.append(mh_mcmc_sampler_res(productions,
                                                             ...
                                                             start=df_prior_labels['rulestring'].mode()[0],
                                                             start_frame=prior_detail_label[str(map_index_label)],
                                                             iter = n_2,
                                                             type='posterior_map',
                                                             prod_l=df_prior_labels['prod_l'][map_index_label],
                                                             prod_d=df_prior_labels['prod_d'][map_index_label],
                                                                         row=len(df_post_map['rulestring']),
                                                             out_penalizer=100)[0])
               #
          ...
          main_data_formatted.to_csv('model_results/normative_res_con_one_' + str(rep) + '_test_2.csv')   # writing main data to new csv file including all relevant data for analysis

          rep+=1

predicted_selections(main_data_formatted, rules_dict, trial_counts, n_rep=1)
