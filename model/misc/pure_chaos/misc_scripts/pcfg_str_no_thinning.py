...
def mh_mcmc_sampler(productions,
    ...
    thin_length = iter  # length of thinned mcmc chain
    ...
    for i in range(iter):
        ...
        # if i % 10 == 0:
        df_rules["rulestring"][int(i)] = t["rule"]
        df_rules["li"][int(i)] = np.prod(t["prec"]["li"])
        df_rules["productions"][int(i)] = t["prod_d"]
        df_rules["n_pds"][int(i)] = len(t["prec"])
        df_rules["prod_l"][int(i)] = t["prod_l"]
        df_rules["prod_d"][int(i)] = t["prod_d"]
        df_rules["results"][int(i)] = z_t
        df_rules["gt_trials"][int(i)] = ground_truth_trials
        rule_details[str(int(i))] = t["prec"]
