...
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=5):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for rrrs in main_data_formatted['post_resp'][:n_rows]:
              # print(rrrs)
              rev_probs.append(np.prod(rr.get_prec_recursively(rr.string_to_list(rrrs))['li']))
          print(rev_probs)
          print(np.mean(rev_probs))

          for rrr in main_data_formatted['prior_resp'][:n_rows]:
              # print(rrr)
              init_probs.append(np.prod(rr.get_prec_recursively(rr.string_to_list(rrr))['li']))
          print(np.mean(init_probs))
          print(init_probs)
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          main_data_formatted.to_csv('model_results/normative_res_cond_three_' + str(rep) + 'checksss.csv')   # writing main data to new csv file including all relevant data for analysis
