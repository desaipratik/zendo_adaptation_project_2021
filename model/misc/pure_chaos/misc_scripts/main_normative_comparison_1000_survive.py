...
# "ob.forall(lambda x1: ob.forall(lambda x2: ob.equal(x1, x2, "'size'"), X), X)"

# summarizing grammar in dictionary
productions = {"S": S, "A": A, "B": B, "C": C, "D": D, "E": E, "G": G, "H": H, "I": I, "J": J, "K": K, "L": L, "M": M}

####################### Sampling Algorithm #########################################
def predicted_selections(main_data_formatted, rules_dict, n_rep=1, n_1=1, n_2=10000):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of iterations)
     ...
     #  looping over each trial for each subject (n_trials * n_subjects iterations and run mcmc chains)
          for data in main_data_formatted['data']:
               ...
               full_training_dat_prior = trials + training_data
               full_training_dat_post_map = generalizations + training_data
               full_training_dat_post_all = full_data + training_data
               truth_partner = eval(main_data_formatted['prior_partner'][i])

               # print(len(full_training_dat_prior))
               # print('hi')
               # print(len(full_training_dat_post_map))
               # print('=no')
               # print(len(full_training_dat_post_all))
               # print('hiwa')


               # creating data frames, one for each different mcmc chain
               # prior chain
               df_prior = mh_mcmc_sampler(productions, full_training_dat_prior, ground_truth=False, iter = 0)[0] # creating empty data for first initial rules
               prior_details = [] # details for prior rules are needed to run the map mcmc chain which starts with the map rule of the prior

               # post map chain
               df_post_map = mh_mcmc_sampler(productions, full_training_dat_post_map, ground_truth=False, iter = 0)[0] # creating empty data for revised rules

               # post all chain
               df_post_all = mh_mcmc_sampler(productions, full_training_dat_post_all , ground_truth=False, iter = 0)[0] # creating empty data for all rules


               # mcmc chains
               # prior
               for sample in range(n_1):
                    prior_df_and_details = mh_mcmc_sampler(productions,
               ...
               for sample in range(n_1):
                    df_post_map = df_post_map.append(mh_mcmc_sampler(productions,
               ...
               for sample in range(n_1):
                    df_post_all = df_post_all.append(mh_mcmc_sampler(productions,
               ...
               for gen in generalizations:        # looping over all 8 generalization scenes
                    ...
                    res_prior = []                # result checks whether this scene follows a rule (length of results equals n_1 * n_2/10)
                    res_post_map = []
                    res_post_all = []
               ...
               if len(fitted_taus_prior) % 5 == 0:
                    ...
                    if len(fitted_taus_prior) % 200 == 0:
                         ...
                         BICs_model_all_subjects_prior.append(-2 * fitted_results_all_subjects_prior[1] + 1 * np.log(8 * len(fitted_taus_prior)))
                         ...
                         BICs_model_all_subjects_post_map.append(-2 * fitted_results_all_subjects_post_map[1] + 1 * np.log(8 * len(fitted_taus_prior)))
                         ...
                         BICs_model_all_subjects_post_all.append(-2 * fitted_results_all_subjects_post_all[1] + 1 * np.log(8 * len(fitted_taus_prior)))


                         # baseline

                         baseline_probs_all_subjects = [.5] * 8 * 200
     ...
     ################ APPENDING ALL DATA TO MAIN DATA FRAME ########################
          # print(select_probs_all_trials_prior)
          ...
          main_data_formatted['fitted_taus_all_subjects_prior'] = fitted_taus_all_subjects_prior *  len(main_data_formatted['rule_name'])
          main_data_formatted['select_probs_all_subjects_prior'] = select_probs_all_subjects_prior *  len(main_data_formatted['rule_name'])
          main_data_formatted['ll_model_all_subjects_prior'] = ll_model_all_subjects_prior *  len(main_data_formatted['rule_name'])
          main_data_formatted['BICs_model_all_subjects_prior'] = BICs_model_all_subjects_prior *  len(main_data_formatted['rule_name'])
          main_data_formatted['ll_baseline_all_subjects_prior'] = ll_baseline_all_subjects_prior * len(main_data_formatted['rule_name'])
          main_data_formatted['BICs_baseline_all_subjects_prior'] = BICs_baseline_all_subjects_prior *  len(main_data_formatted['rule_name'])
          ...
          main_data_formatted['fitted_taus_all_subjects_post_map'] = fitted_taus_all_subjects_post_map *  len(main_data_formatted['rule_name'])
          main_data_formatted['select_probs_all_subjects_post_map'] = select_probs_all_subjects_post_map * len(main_data_formatted['rule_name'])
          main_data_formatted['ll_model_all_subjects_post_map'] = ll_model_all_subjects_post_map *  len(main_data_formatted['rule_name'])
          main_data_formatted['BICs_model_all_subjects_post_map'] = BICs_model_all_subjects_post_map * len(main_data_formatted['rule_name'])
          main_data_formatted['ll_baseline_all_subjects_post_map'] = ll_baseline_all_subjects_prior * len(main_data_formatted['rule_name'])
          main_data_formatted['BICs_baseline_all_subjects_post_map'] = BICs_baseline_all_subjects_prior * len(main_data_formatted['rule_name'])
          ...
          main_data_formatted['fitted_taus_all_subjects_post_all'] = fitted_taus_all_subjects_post_all *  len(main_data_formatted['rule_name'])
          main_data_formatted['select_probs_all_subjects_post_all'] = select_probs_all_subjects_post_all * len(main_data_formatted['rule_name'])
          main_data_formatted['ll_model_all_subjects_post_all'] = ll_model_all_subjects_post_all *  len(main_data_formatted['rule_name'])
          main_data_formatted['BICs_model_all_subjects_post_all'] = BICs_model_all_subjects_post_all *  len(main_data_formatted['rule_name'])
          main_data_formatted['ll_baseline_all_subjects_post_all'] = ll_baseline_all_subjects_prior *  len(main_data_formatted['rule_name'])
          main_data_formatted['BICs_baseline_all_subjects_post_all'] = BICs_baseline_all_subjects_prior *  len(main_data_formatted['rule_name'])
          ...
          main_data_formatted.to_csv('full_comparison_1000_survive/8_training_data/full_comparison_5_penalisation' + str(rep) + '.csv')   # writing main data to new csv file including all relevant data for analysis



predicted_selections(main_data_formatted,rules_dict,n_rep=10)
