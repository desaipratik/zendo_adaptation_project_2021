...
def ibs_mcmc_sampler(productions, data, ground_truth = None, start="S",
                    start_frame=pd.DataFrame({"from": [], "to": [], "toix": [], "li": []}),rule=None):
    ...
    t = ob.generate(productions, start, [], start_frame)  # create an initial rule t

    df_rules = pd.DataFrame({"rulestring": [],   # data frame for storing rules and other relevant properties
                             "productions": [],
                             "prod_l": [],
                             "li": [],
                             "n_pds": [],
...
<<<<<<< HEAD
    ...
    #
    #     t_prime = ob.generate(productions, sub_t[0], sub_t[1], t["prec"].loc[
    #                                                            0:len_sub_tree - 1:])  # arguments: 1) sub_t[0] = sub rule, 2) sub_t[1] = sub bound vars,
    #     # 3) t["prec"].loc[0:len_sub_tree-1:] = properties of rule created by sub tree
    #
    #     new_sub_tree = t_prime[
    #         "prod_d"]  # new subtree including novel productions following the previously fixed sub tree
    ...
    #     t_prime["prod_l"] = ob.probs_list(reproduce_full_prod_dict(new_parse_tree, productions))   # same for production list
        ...
        t_prime = ob.generate(productions, start, [], start_frame)  # create an initial rule t
        ...
        for i_2 in range(0, len(data)):   # first looping over all 8 scenese
            ...
            for i_3 in range(0, len(data[i_2]['ids'])):  # now looping over the number of objects in each scene
                object = {"id": data[i_2]['ids'][i_3], "colour":  data[i_2]['colours'][i_3], "size":  data[i_2]['sizes'][i_3], "xpos":  data[i_2]['xpos'][i_3],
                     "ypos":  data[i_2]['ypos'][i_3], "rotation":  data[i_2]['rotations'][i_3], "orientation":  data[i_2]['orientations'][i_3],
        ...
        z_t = [a and b for a, b in zip(ground_truth, res_t)]  # comparing results with ground truth
        z_t_prime = [a and b for a, b in zip(ground_truth, res_t_prime)]
        ...
        ll_t = ll.compute_ll(1, 2, out_t)  # note that number 1 and 2 are currently constants that will be replaced by variables in due course
        ll_t_prime = ll.compute_ll(1, 2, out_t_prime)

        # 4) computing derivation probability of rule
        ...
        p_acceptance = (ll_t_prime / ll_t) * (p_t / p_t_prime) * (nt_t / nt_t_prime) * (deriv_t / deriv_t_prime)
        ...
        if np.random.rand() < p_acceptance:
            ...
            res_t = res_t_prime  # and the results are the results from the new rule

        # appending t
        df_rules_trial = pd.DataFrame({"rulestring": t["rule"],   # data frame for storing rules and other relevant properties
                         "productions": [t["prod_d"]],
                         "prod_l": [t["prod_l"]],
                         "li": np.prod(t["prec"]["li"]),
                         "n_pds": len(t["prec"]),
                         "results": [res_t]})

        df_rules = df_rules.append(df_rules_trial, ignore_index=True)
...
=======
    ...
    print(t['rule'])
    print(rule)
>>>>>>> 04c2f29b1cdddb9be6d35ec84031321f6ebe8441
