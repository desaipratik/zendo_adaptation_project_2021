...
# #           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('tr_counts_only_change'+str(rep)+'_poiss_lamb3_cond2.txt', 'w') as f_tr:
              for item_tr in n_iter_tr:
                  f_tr.write("%s\n" % item_tr)

          with open('ts_counts_only_change'+str(rep)+'_poiss_lamb3_cond2.txt', 'w') as f_ts:
              for item_ts in n_iter_ts:
                  f_ts.write("%s\n" % item_ts)



          main_data_formatted.to_csv('model_results/normative_res_twop_' + str(rep) + '_process_models_inverse_binomial_only_change_lamb3.csv')   # writing main data to new csv file including all relevant data for analysis
