...
main_data_formatted = pd.read_csv('../data/main_data_cond_one.csv')  # getting the preprocessed data file
...
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=5):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data_prior'][:n_rows]:
               ...
               resample_probs.append(wsls_check)
...
#           # print(raw_probs_all_trials_one_list_prior)
          ...
          with open('wsls'+str(rep)+'_resample_probs.txt', 'w') as f_tr:
              for rprob in resample_probs:
                  f_tr.write("%s\n" % rprob)
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          #
          # with open('model_results/test4/rules_prior_chain_clean_poiss_3_bn.txt', 'w') as filehandle:
          ...
          # with open('model_results/test4/rules_post_map_chain_clean_qual_lambda3_bn.txt', 'w') as filehandle:
          ...
          # with open('model_results/test4/rules_post_all_seed_chain_clean_qual_lambda3_bn.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_post_all_seed)
          #
          #
          #
          main_data_formatted.to_csv('normative_res_one_' + str(rep) + '_process_models_qual_fit_poiss_lambada1bn_2.csv')   # writing main data to new csv file including all relevant data for analysis
