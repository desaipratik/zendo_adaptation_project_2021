...
class pcfg_generator(rules):
    ...
    def generate_res(self,
                 productions=None,
                 start="S",
                 prec=pd.DataFrame({"from": [], "to": [], "toix": [], "li": []}),
                 bound_vars=[],
                 ...
                 Mwin = [0.333333333,.333333333,.333333333]):
        """ generates a rule accordang to the specified production, the start rule (rule argument), the start bound
            ...
            for i in range(0, len(srule)):
                ...
                if srule[i] == 'C':
                    ...
                    if len(bound_vars) < 2:
                        ...
                        # print(bound_vars)
                        # bound_vars = corr_bvs[str(bound_vars[-1])]
