...
# # #         # print(t_prime['rule'])
        ...
        if type == 'prior':
            ...
            for i_2 in range(0, len(data)):
                ...
                ground_truth.append(ground_truth_labels[i_2])
                res_t_prime.append(eval(t_prime["rule"]))
...
# #         print(t['rule'])
        ...
        t = Z.generate_res(productions=productions, start="S",prec=start_frame,bound_vars=[],feat_probs=feat_probs,Dwin=Dwin)
    ...
    #         # ['rule'])
        ...
        for i_6 in range(0, len(test_data)):
            ...
            for i_7 in range(0, len(test_data[i_6]['ids'])):  # looping over the number of objects in each scene
                ...
                X.append(object)   # appending the object to X which includes all objects for a scene
            # res_t.append(eval(t["rule"]))             # evaluating the rules against X
            res_t_rule.append(eval(t["rule"]))
            res_rev_rule.append(eval(rev_rule))
#     #         # print(ground_truth)

        if res_t_rule==res_rev_rule:
            print(res_t_rule)
            print(i_edit)
            print('gotcha ts')
    ...
    df_rules = df_rules.dropna()
    return [df_rules, rule_details, n_iter,n_edit]
...
# # #         # print(t['rule'])
        ...
        edits = np.random.poisson(lam=3) + 1
        # edits = 15
        n_edit_count = 0
        for i_edit in range(edits):
            n_edit_count+=1
...
# #     #             # print('hi')
            ...
            for i_4 in range(8, len(data)):
                ...
                res_t_process.append(eval(t["rule"]))             # evaluating the rules against X
                res_t_prime_process.append(eval(t_prime["rule"]))
                ground_truth_trials_init.append(data[i_4]['follow_rule'])

            z_t_process = [a and b or not a and not b for a, b in zip(ground_truth_trials_init, res_t_process)]  # comparing results with ground truth
            z_t_prime_process = [a and b or not a and not b for a, b in zip(ground_truth_trials_init, res_t_prime_process)]
