"""
Created on Sun Mar 15 15:01:821 2020
...
main_data_formatted = pd.read_csv('model_results/final_res_csv/normative_res_three_10k_second_rule_only.csv')  # getting the preprocessed data file
...
init_probs = []
rev_probs = []
for rrrs in main_data_formatted['post_resp'][:82]:
...
for rrr in main_data_formatted['prior_resp'][:82]:
...
# print(equalities_i[:82])
# print(list(main_data_formatted['prior_resp'][:82]))
...
with open('model_results/test6/rules_post_all_wsls_damn.txt', 'r') as filehandle:
...
for wsls_rule_list,init_resp in zip(wsls_rules[:82],main_data_formatted['prior_resp'][:82]):
  rev_probs_wsls_all = []
...
with open('model_results/test6/rules_post_all_seed_lamb3c3.txt', 'r') as filehandle:
    filecontents = filehandle.readlines()
    for line in filecontents:
        # remove linebreak which is the last character of the string
        current_place = line[:-1]
        # add item to the list
        tr_rules.append(current_place)


for tr_rule_list,init_resp in zip(tr_rules[:82],main_data_formatted['prior_resp'][:82]):
  rev_probs_wsls_all = []
  q_r_specific_wsls_all = []
  b_r_specific_wsls_all = []
  e_r_specific_wsls_all = []
  ...
  for tr_rule in eval(tr_rule_list):
    rev_probs_wsls_all.append(np.log(np.prod(rr.get_prec_recursively(rr.string_to_list(tr_rule))['li'])))
    from_list = list(rr.get_prec_recursively(rr.string_to_list(tr_rule))['from'])
    to_list = list(rr.get_prec_recursively(rr.string_to_list(tr_rule))['to'])
...
with open('model_results/test6/rules_post_map_lamb3c3.txt', 'r') as filehandle:
    filecontents = filehandle.readlines()
    for line in filecontents:
        # remove linebreak which is the last character of the string
        current_place = line[:-1]
...
with open('model_results/norm_3/rules_post_all.txt', 'r') as filehandle:
    filecontents = filehandle.readlines()
    for line in filecontents:
        # remove linebreak which is the last character of the string
        current_place = line[:-1]
...
with open('model_results/norm_3/rules_prior.txt', 'r') as filehandle:
    filecontents = filehandle.readlines()
    for line in filecontents:
        # remove linebreak which is the last character of the string
        current_place = line[:-1]
...
with open('model_results/norm_3/rules_post_all.txt', 'r') as filehandle:
    ...
    for line in filecontents:
        ...
        current_place = line[:-1]
        # add item to the list
        wsls_rules_post.append(current_place)


for wsls_rule_list,rev_resp in zip(wsls_rules[:82],wsls_rules_post[:82]):
  rev_probs_wsls_all = []
  q_r_specific_wsls_all = []
  b_r_specific_wsls_all = []
  e_r_specific_wsls_all = []
...
print(np.mean(q_shared_all_tnp))
...
rule_comps.to_csv('model_results/rule_comp_cond_3_lamb3.csv')
