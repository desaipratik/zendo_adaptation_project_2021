...
####################### Custom imports ########################################
from mcmc_cond_1_new_test_change_seq_ib_poiss_3 import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
#                # print(rule_string)
               ...
               subj_rev_rule = rr.list_to_string(rr.string_to_list(subj_rev_rule))
               ...
               with open('1000_random_subj_scenes.txt', 'r') as filehandle:
                   filecontents = filehandle.readlines()
                   for line in filecontents:
                       current_place = line[:-1]
                       test_training_data.append(eval(current_place))

               test_training_data = random.sample(test_training_data,100)
...
# #                # print(df_prior['rulestring'])
    ...
    #     #                     # print(df_post_map)
    #     #                     # print('dropped abvoe ')
                        all_post_map_dat = mcmc_sampler_map_surgery(productions,replacements,
                                                                 full_training_dat_post_map,
                                                                                  rev_rule=subj_rev_rule,
                                                                 Dwin=Dwin_post,
                        ...
                        df_post_map = df_post_map.append(all_post_map_dat[0])
                        print(df_post_map)

                        n_iter_ts.append(all_post_map_dat[2])
...
# #                # print(df_post_all)
               ...
               if subj_rule != subj_rev_rule:
                   for sample in range(n_1):
                        all_seed_dat =  mcmc_sampler_map(productions, replacements,full_training_dat_post_map,
                                                                                start=subj_rule,
                                                                                    rev_rule=subj_rev_rule,
                                                                                bv=bv_prior,
                                                                                ground_truth_labels=prior_response,
                        ...
                        df_post_all_seed = df_post_all_seed.append(all_seed_dat[0])
                        print(df_post_all_seed)
                        print(all_seed_dat)
                        n_iter_tr.append(all_seed_dat[2])
               else:
...
# #           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('tr_counts_only_change_5'+str(rep)+'.txt', 'w') as f_tr:
              for item_tr in n_iter_tr:
                  f_tr.write("%s\n" % item_tr)

          with open('ts_counts_only_change_5'+str(rep)+'.txt', 'w') as f_ts:
              for item_ts in n_iter_ts:
                  f_ts.write("%s\n" % item_ts)



          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_process_models_inverse_binomial_only_change_5.csv')   # writing main data to new csv file including all relevant data for analysis

          rep+=1

predicted_selections(main_data_formatted, rules_dict, replacements, trial_counts, n_rep=100)
...
####################### Custom imports ########################################
from mcmc_cond_1_new_test_change_seq_ib import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
from pcfg_generator import pcfg_generator
from tree_regrower import tree_regrower
from transform_functions import compute_orientation, check_structure, get_production_probs_prototype
from tau_ml_estimation import get_tau, fitted_probs, compare_BIC, hard_max_selections, compute_acc, compute_distance
...
# #           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('tr_counts_only_change'+str(rep)+'_poiss_lamb3_subj_scenes.txt', 'w') as f_tr:
              for item_tr in n_iter_tr:
                  f_tr.write("%s\n" % item_tr)

          with open('ts_counts_only_change'+str(rep)+'_poiss_lamb3_subj_scenes.txt', 'w') as f_ts:
              for item_ts in n_iter_ts:
                  f_ts.write("%s\n" % item_ts)



          main_data_formatted.to_csv('model_results/normative_res_one_' + str(rep) + '_process_models_inverse_binomial_only_change_lamb3_subj_scenes.csv')   # writing main data to new csv file including all relevant data for analysis
