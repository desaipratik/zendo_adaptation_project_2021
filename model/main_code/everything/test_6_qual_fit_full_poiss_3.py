...
####################### Custom imports ########################################
from mcmc_cond_3_new_test_change_seq_poiss_3 import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=5):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data'][:n_rows]:
               ...
               full_training_dat_post_map = generalizations + init_trials
               ...
               prior_rule_strings_label = rule_string
...
#                # print(df_prior['rulestring'])
               row_count = 0
               rand_n_check = np.random.rand()
#                # print(df_prior['rulestring'])
               if (1 - wsls_check) > rand_n_check:
                   for sample in range(1):
    #                     # print('cat')
    #                     # print('qhale')
...
#                # print(df_post_all)
               ...
               if (1 - wsls_check) > rand_n_check:
                   ...
                   df_post_all_seed = pd.DataFrame({'rulestring': [subj_rule] * 1000})
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          # with open('model_results/test6/rules_prior_label_lamb3c3.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_prior_label)
          with open('model_results/test6/rules_post_map_lamb3c3_single_edit.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_map)
          with open('model_results/test6/rules_post_all_wsls_damn_single_edit.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all)
          with open('model_results/test6/rules_post_all_seed_lamb3c3_single_edit.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all_seed)



          main_data_formatted.to_csv('model_results/normative_res_cond_three_' + str(rep) + '10k_process_models_single_edit.csv')   # writing main data to new csv file including all relevant data for analysis
