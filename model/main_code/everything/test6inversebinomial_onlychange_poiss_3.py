...
####################### Custom imports ########################################
from mcmc_cond_3_new_test_change_seq_ib_poiss_3 import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
#                if (1 - wsls_check) > rand_n_check:
    ...
    #     #                     # print('dropped abvoe ')
                        all_post_map_dat = mcmc_sampler_map_surgery(productions,replacements,
                                                                 ...
                                                                 # iter_1 = n_1,
                                                                                 ground_truth_labels=true_partner,
...
# #                # print(df_post_map['rulestring'])
               max_a_posteriori_post_map = subj_rule
...
# #                # print(df_post_all)
               ...
               #
               max_a_posteriori_post_all_seed =subj_rule
...
#                     print(sum(res_prior_label))
                    ...
                    p_follow_rule_post_map = (1 / 3) * 1 # len(res) = number of rules; sum(res) = number of rules matching the scene
                    ...
                    p_follow_rule_post_all_seed = (1 / 2) * 2# len(res) = number of rules; sum(res) = number of rules matching the scene
...
# #           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('tr_counts'+str(rep)+'_poiss_3_1k_cond3_onlychange.txt', 'w') as f_tr:
              for item_tr in n_iter_tr:
                  f_tr.write("%s\n" % item_tr)

          with open('ts_counts'+str(rep)+'_poiss_3_1k_cond3_onlychange.txt', 'w') as f_ts:
              for item_ts in n_iter_ts:
                  f_ts.write("%s\n" % item_ts)


          main_data_formatted.to_csv('model_results/normative_res_three_' + str(rep) + '_process_models_inverse_onlychange_15.csv')   # writing main data to new csv file including all relevant data for analysis
