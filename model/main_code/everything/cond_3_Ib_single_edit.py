...
####################### Custom imports ########################################
from mcmc_cond_3_new_test_change_seq_ib_single_step import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
# #           # print(BICs_baseline_all_subjects_prior)
          ...
          with open('tr_counts'+str(rep)+'_cond_3_single_edit_final_reps.txt', 'w') as f_tr:
              for item_tr in n_iter_tr:
                  f_tr.write("%s\n" % item_tr)

          with open('ts_counts'+str(rep)+'_cond_3_single_edit_final_reps.txt', 'w') as f_ts:
              for item_ts in n_iter_ts:
                  f_ts.write("%s\n" % item_ts)



          # main_data_formatted.to_csv('model_results/normative_res_three_' + str(rep) + '_process_models_inverse_binomial_15.csv')   # writing main data to new csv file including all relevant data for analysis

          rep+=1

predicted_selections(main_data_formatted, rules_dict, replacements, trial_counts, n_rep=10)
