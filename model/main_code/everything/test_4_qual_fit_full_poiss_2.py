...
####################### Custom imports ########################################
from mcmc_cond_1_new_test_change_seq_poiss_2 import mcmc_sampler, mcmc_sampler_map_surgery, mcmc_sampler_map
...
main_data_formatted = pd.read_csv('../model_fit_rev_generalizations/normative_res_one_0_process_models_chain.csv')  # getting the preprocessed data file
...
def predicted_selections(main_data_formatted, rules_dict,  replacements, trial_counts, n_rep = 1, n_1=1, n_2=5):  # computes the ll for getting participants responses to initial generalizations (n_1 * n_2 determines number of MCMC iterations)
     ...
     for repeat in np.arange(n_rep):     # for each repeat, an independent outuput file will be created
          ...
          for data in main_data_formatted['data_prior'][:n_rows]:
               ...
               full_training_dat_post_map = rev_trials + init_trials
               ...
               prior_rule_strings_label = [subj_rule] * 1
...
#                # print(df_prior['rulestring'])
               ...
               n_edits =  np.random.poisson(lam=3) + 1
#                # print(df_prior['rulestring'])
    ...
    #                     # print('dropped abvoe ')
                   ...
                   # acceptance_probs_sort = acceptance_probs.copy()
                   # acceptance_probs_sort.sort()
                   # acc_clean = []
                   # for acc_prob in acceptance_probs_sort:
                   #     if acc_prob not in acc_clean:
                   #         acc_clean.append(acc_prob)
                   # s_max_score = acc_clean[-2]
                   #
                   # t_max_score = acc_clean[-3]


                   # max_score = max(acceptance_probs)
                   # max_scores = [index for index, x in enumerate(acceptance_probs) if x == max_score]
                   # max_scores_2 = [index for index, x in enumerate(acceptance_probs) if x == s_max_score]
                   # max_scores_3 = [index for index, x in enumerate(acceptance_probs) if x == t_max_score]
                   # remaning_rules = []
                   # for mx_sc in max_scores:
                   #     remaning_rules.append(df_post_map['rulestring'][mx_sc])
                   # for mx_sc_2 in max_scores_2:
                   #    remaning_rules.append(df_post_map['rulestring'][mx_sc_2])
                   # for mx_sc_3 in max_scores_3:
                   #    remaning_rules.append(df_post_map['rulestring'][mx_sc_3])
                   #
                   #
                   #
                   #
                   # mode_map = df_post_map['rulestring'].mode()[0]
                   # remaning_rules.append(subj_rule)
                   # print(remaning_rules)
                   # df_post_map = pd.DataFrame({'rulestring': remaning_rules})
               else:
...
#                # print(df_post_map['rulestring'])
               ...
               #this is the post all winstaylosesample
               ############################################
               if (1 - wsls_check) > rand_n_check:
...
#                # print(df_post_all)
               ...
               if (1 - wsls_check) > rand_n_check:
                   ...
                   # remaning_rules = []
                   # for mx_sc in max_scores:
                   #     remaning_rules.append(df_post_all_seed['rulestring'][mx_sc])
                   # # for mx_sc_2 in max_scores_2:
                   # #    remaning_rules.append(df_post_all_seed['rulestring'][mx_sc_2])
                   # remaning_rules.append(subj_rule)



                   # print(remaning_rules)
                   # df_post_all_seed = pd.DataFrame({'rulestring': remaning_rules})
               else:
...
#                # print(df_post_all)
               ...
               # print(post_all_rule_strings_seed[:10])
...
#                     # print(p_follow_rule_prior)
               ...
               fitted_tau_post_all = minimize(get_tau, 1, bounds=[(0.1, 100.00)],args=(prob_gen_follow_rule_post_all,posterior_response), method='L-BFGS-B')
...
#           # print(BICs_baseline_all_subjects_prior)
          ...
          #
          # with open('model_results/test4/rules_prior_chain_clean.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_prior)
          # with open('model_results/test4/rules_prior_label_chain_clean.txt', 'w') as filehandle:
          #     filehandle.writelines("%s\n" % place for place in rules_prior_label)
          with open('rules_post_map_chain_clean_qual_lambda1_bn_old.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_map)
          with open('rules_post_all_chain_clean_bn.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all)
          with open('rules_post_all_seed_chain_clean_qual_lambda1_bm_old.txt', 'w') as filehandle:
              filehandle.writelines("%s\n" % place for place in rules_post_all_seed)



          main_data_formatted.to_csv('normative_res_one_' + str(rep) + '_pr2ocess_models_qual_fit_poiss_lambda3_bn_old_data_finalc.csv')   # writing main data to new csv file including all relevant data for analysis
