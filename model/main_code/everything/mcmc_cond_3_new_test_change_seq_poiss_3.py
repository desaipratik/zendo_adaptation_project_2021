...
# #         # print(t_prime['rule'])
        ...
        if type == 'prior':
            ...
            for i_2 in range(0, len(data)):
                ...
                ground_truth.append(ground_truth_labels[i])
                res_t_prime.append(eval(t_prime["rule"]))
        ...
        # doing this for all 16 data points together
            ...
            for i_4 in range(8, len(data)):
                ...
                # print(i_4)
                ground_truth_trials_rev.append(ground_truth_labels[i_4-8])
...
#         print(t['rule'])
        ...
        edits = 1 #np.random.poisson(lam=3) + 1

        # print('caca')
        for i_edit in range(edits):
            # print('thisisedits')
...
#     #         # print(t_prime)
            ...
            ground_truth = ground_truth_labels[:8]
            ground_truth_self = ground_truth_labels[8:]
            ...
            z_t_process = [a and b or not a and not b for a, b in zip(ground_truth_self, res_t_process)]  # comparing results with ground truth
            z_t_prime_process = [a and b or not a and not b for a, b in zip(ground_truth_self, res_t_prime_process)]

            out_t_process = len(z_t_process) - sum(z_t_process)  # counts the number of false classifications for a rule
            out_t_prime_process = len(z_t_prime_process) - sum(z_t_prime_process)

            z_t = [a and b or not a and not b for a, b in zip(ground_truth_labels, res_t)]  # comparing results with ground truth
            z_t_prime = [a and b or not a and not b for a, b in zip(ground_truth_labels, res_t_prime)]
...
#     #         # print(ground_truth)
            ...
            if np.log(np.random.rand()) < p_acceptance and sum(res_test) != len(res_test):
#     #             # print(p_acceptance)
                t = t_prime
                z_t = z_t_prime
                check_val = 1
...
# #         # print(t['rule'])
        ...
        edits = 1 # p.random.poisson(lam=3) + 1
...
# #         # print(t_prime['rule'])
        ...
        # 3a) first need to evaluate the rules and check how many outliers they have based on ground truth
            ...
            # z_t_process = [a and b or not a and not b for a, b in zip(ground_truth_labels_self, res_t_process)]  # comparing results with ground truth
            # z_t_prime_process = [a and b or not a and not b for a, b in zip(ground_truth_, res_t_prime_process)]
